...
class VisionTransformer(torch.nn.Module):
    ...
    def prepare(self, x):
        ...
        if self.use_cls_token:
            cls_tokens = self.cls_token.expand(batch_size * view_num, -1, -1)  # [B*V, 1, D]
            x = torch.cat((cls_tokens, x), dim=1)  # [B*V, P+1, D]
            x = x + self.pos_embed
        else:
...
class DistilledVisionTransformer(VisionTransformer):
    ...
    def prepare(self, x):
        ...
        if self.use_cls_token:
            cls_tokens = self.cls_token.expand(batch_size * view_num, -1, -1)  # [B*V, 1, D]
            dist_token = self.dist_token.expand(batch_size * view_num, -1, -1)  # [B*V, 1, D]
            x = torch.cat((cls_tokens, dist_token, x), dim=1)  # [B*V, P, D]
            x = x + self.pos_embed
        else:
